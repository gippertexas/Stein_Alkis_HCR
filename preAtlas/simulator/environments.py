...
for file in os.listdir('data1/assets/furnitures'):
    if os.path.isdir(os.path.join('data1', 'assets', 'furnitures', file)):
...
for file in os.listdir('data1/assets/objects'):
    if os.path.isdir(os.path.join('data1', 'assets', 'objects', file)):
...
class PerceptionWrapper(gym.Wrapper):
    ...
    def _set_world(self, setup_obstacles):
        ...
        #assets['ground'] = self.get_ground()

        mass = 0

        idColWall = self.createCollisionShape(self.GEOM_PLANE)
        assets['walls'] = [ self.createMultiBody(mass, idColWall, -1, [0.5 * FIELD_RANGE[0],    + 0.5*FIELD_RANGE[1] + 0.2, 1.0], [ 0.707, 0, 0,  0.707]),
                            self.createMultiBody(mass, idColWall, -1, [0.5 * FIELD_RANGE[0],    - 0.5*FIELD_RANGE[1] - 0.2, 1.0], [ 0.707, 0, 0, -0.707]),
                            self.createMultiBody(mass, idColWall, -1, [0.0,                       0.0,                      2.0], [     1, 0, 0,  0])]

        #self.changeVisualShape(assets['ground'], -1, textureUniqueId=self.loadTexture("data1/textures/carpet.png"))
        self.changeVisualShape(assets['walls'][0], -1, textureUniqueId=self.loadTexture("data1/textures/wall.png"), rgbaColor=(0.8, 0.7, 0.8, 1.0))
        self.changeVisualShape(assets['walls'][1], -1, textureUniqueId=self.loadTexture("data1/textures/wall.png"), rgbaColor=(0.8, 0.7, 0.8, 1.0))
        self.changeVisualShape(assets['walls'][2], -1, textureUniqueId=self.loadTexture("data1/textures/wall.png"), rgbaColor=(0.6, 0.7, 0.9, 1.0))
        ...
        for dict_item in setup_obstacles['furnitures']:
            ...
            assets['furnitures'].append(self.loadURDF(path, basePosition, baseOrientation, globalScaling = scale, useFixedBase = True))
        ...
        for dict_item in setup_obstacles['objects']:
            ...
            scale = object['scale']
            path = object['urdfPath']
            assets['objects'].append(self.loadURDF(path, basePosition, baseOrientation, globalScaling = scale, useFixedBase = False))

        for dict_item in setup_obstacles['people']:
            basePosition = dict_item['position']
            baseOrientation = dict_item['orientation']
            scale = dict_item['scale']
            assets['people'].append(ObjectHuman(self, basePosition, baseOrientation, scale))

        #assets['agent'] = self.robot.quadruped
    ...
    def _generate_obstacle_profiles(self, numPeople, numObjects, numFurnitures, lenUnit=50, lenOffset=2):
        ...
        for cntUnit in range(int((FIELD_RANGE[0]-lenOffset)/lenUnit)):
            ...
            for idx in range(numFurnitures):
                ...
                dict_item['orientation'] = self.pybullet_client.getQuaternionFromEuler(
            ...
            for idx in range(numPeople):
                ...
                dict_item['position'] =  np.array([lenOffset + lenUnit * (cntUnit + raRandPos[idx,0]), 
                                        FIELD_RANGE[1] *(raRandPos[idx,1] - 0.5),
                                        0.])
                dict_item['orientation'] = self.pybullet_client.getQuaternionFromEuler(valRandYaw[idx] * np.array([0, 0, 2*np.pi]))
